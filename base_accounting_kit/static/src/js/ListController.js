...
export class AccountMoveLineListController extends ListController {
     ...
     async openRecord(record, options = {}) {
        const doc = this.__owl__.bdom.parentEl.ownerDocument;
        const mainKanbanDiv = doc.querySelector("#base_accounting_reconcile");
        // Contexte lettrage bancaire du kit : tbody dédié + id ligne relevé (contexte ou carte kanban).
        // Ne pas utiliser le premier tr[data-id] : ce sont souvent des account.move.line.
        const ctx = this.props.context || {};
        let data_id = Number.parseInt(String(ctx.default_st_line_id ?? ""), 10);
        if (!Number.isFinite(data_id) || data_id <= 0) {
            const kanbanCard =
                doc.querySelector(".o_bank_reconcile_st_line_kanban_card.div-added") ||
                doc.querySelector(".o_bank_reconcile_st_line_kanban_card");
            const fromCard = kanbanCard?.getAttribute("data-oe-id");
            data_id = Number.parseInt(String(fromCard ?? ""), 10);
        }
        if (!mainKanbanDiv || !Number.isFinite(data_id) || data_id <= 0) {
        ...
        const rowResId = record.resId ?? record.id;
        const rowSelector = this.__owl__.bdom.parentEl.querySelector(`tr[data-id='${rowResId}']`)
         if (rowSelector) {
        ...
        this.updateResIdList();
            ...
            console.warn("base_accounting_kit: openRecord fallback", e);
            return super.openRecord(record, options);
        }
    }
